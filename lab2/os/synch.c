...
cond_t CondCreate(lock_t lock) {
  ...
  intrval = DisableIntrs();
  //grab a condition variable
  //use a for loop to check for one that is not in use
  for (i = 0; i < MAX_CONDS; i++) 
  {
	if (conds[i].inuse = 0) {
		cond = conds[i];
		conds[i].inuse = 1;
		//associate lock with it
		//not sure if lock is correct - used lock not lock_t in struct
		cond -> cond_lock = lock;
		break;
	}
  }
  ...
  if (i == MAX_CONDS) {
  	dbprintf ('p', "No condition variables left\n");
	return INVALID_COND;
  }
  ...
  if (AQueueInit (&conds[cond]->waiting) != QUEUE_SUCCESS) {
	printf("could not init waiting queue for conditional variables\n");
	exitsim();
  }
...
int CondHandleWait(cond_t c) {
  //YA IDK WTF THIS IS
  // Your code goes here
  // call process block on condition variable until signal or broadcast is received
  // the lock handle needs to be stored somewhere for this function to work...
  // if the calling process has not acquired the lock return 1
  // if the call was successful return 0
  ...
  if (!&conds[c]) return SYNC_FAIL;

  //make sure that this process ID has the lock
  uint32 currentPID = GetCurrentPid();
  uint32 lockPID = locks[&conds[c] -> lock].pid;
  if (currentPID != lockPID) {
	printf("Current process does not have the needed lock\n");
  ...
  if ((LockHandleRelease(&conds[c] -> lock != SYNC_SUCCESS) {
      printf("Failed to release lock in condWait before going to bed\n");
      exitsim();
  }
  ...
  if (LockHandleAcquire(&conds[c]->lock != SYNC_SUCCESS) {
  	printf("Failed to acquire lock in condWait after putting process to sleep\n");
        exitsim();
  }
...
int CondHandleSignal(cond_t c) {
  // Your code goes here
  // Base off semaphore but also check for lock being acquired by correct ID
  if (c < 0) return SYNC_FAIL;
  if (c >= MAX_CONDS) return SYNC_FAIL;
  if (!conds[c].inuse) return SYNC_FAIL;
  ...
  //checking for the conditional variables existing
  if (!conds[c]) return SYNC_FAIL;


  //check that current process has the lock
  ...
  //check if there is a process to wake up
  if (!AQueueEmpty(&cond->waiting)) {
	l = AQueueFirst(&cond->waiting);
 	pcb = (PCB *)AQueueObject(l);
        if (AQueueRemove(&l) != QUEUE_SUCCESS) {
 		printf("Fatal error removing from QUEUE\n");
		exitsim();
	}
